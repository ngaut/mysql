...
void THD::sqllog_cleanup()
{
  if (sqllog_cache != NULL)
  {
    ...
    my_free((uchar *) sqllog_cache, MYF(0));
    sqllog_cache = NULL;
  }
}
...
{
  /*
    If we are inside the slave replication thread, we can output the primary
    information for this particular event.
  */
  if (thd->slave_thread)
  {
    DBUG_ASSERT(opt_sql_log && !opt_sql_log_as_master);
  ...
  else
  {
    ...
    /* It does not record its own information. */
    strcpy(master_hostname, "testing");
    strcpy(master_log_name, "testing");
    strcpy(master_log_offset, "-1");
  }
}
...
int THD::sqllog_commit()
{
  ...
  if (opt_sql_log && current_event_seq > 0 && sqllog_cache != NULL)
  {
    ...
    char group_id_to_write[21]= "-1";
    if (slave_thread)
    {
      if (rpl_hierarchical)
        sprintf(group_id_to_write, "%llu", (unsigned long long) group_id);
      else
        sprintf(group_id_to_write, "%d", 0);
    }
    /*
    ...
    */
    ...
    middle_columns_length= snprintf((char *) middle_columns, middle_columns_max,
                                    ...
                                    t.tm_min, t.tm_sec, group_id_to_write,
                                    current_event_seq);
