...
static bool test_if_use_dynamic_range_scan(JOIN_TAB *tab);
...
JOIN::optimize()
{
  ...
  */
  ...
  /*
    ...
    Yet the current implementation of FORCE INDEX hints does not allow
    us to do it in a clean manner.
  */
...
make_join_select(JOIN *join,SQL_SELECT *select,COND *cond)
{
  ...
  if (select)
  {
	...
	join->best_positions[i].records_read= rows2double(tab->quick->records);
      ...
      // TODO(gpang): 6.0 added lots more conditions.
	...
	*/        
        if (cond && tmp)
        {
          ...
          */
          ...
          // TODO(gpang): 6.0 added (&& !first_inner_tab)
                            ...
                            uint no_jbuf_after)
{
  ...
  bool allow_default_mrr_bka= join->thd->variables.allow_default_mrr_bka;
  ...
  switch (tab->type) {
  case JT_ALL:
    // TODO(gpang): in 6.0 (sj_inner_tab?)
  ...
  case JT_EQ_REF:
    if (cache_level <= 4)
      return FALSE;
    /*
      Only use BKA with GoogleStats tables.
    */
    if (opt_restrict_bka_to_googlestats && tab->table->file->ht &&
        tab->table->file->ht->db_type != DB_TYPE_GOOGLESTATS)
    {
      goto no_join_cache;
    }
...
sub_select_cache(JOIN *join,JOIN_TAB *join_tab,bool end_of_records)
{
  ...
  if (!test_if_use_dynamic_range_scan(join_tab))
  {
    ...
    /*
      We have decided that after the record we've just put into the buffer,
      we won't add more records. Now try to find all the matching
      extensions for all records in the buffer.
    */
    return cache->join_records(FALSE);
  }
  /* TODO(gpang?): Check whether we need the call below. If not, remove it. */
...
static bool
test_if_use_dynamic_range_scan(JOIN_TAB *join_tab)
{
  ...
  if (blobs)
  {
    ...
    for ( ; copy_ptr < copy_ptr_end; copy_ptr++)
    {
      ...
      if (!blob_field->is_null())
      {
        ...
        len+= blob_len;
        blob_field->get_ptr((uchar**)(&(*copy_ptr)->str));
      }
    }
  }
  ...
  for ( ; copy < copy_end; copy++)
  {
    ...
    if (copy->type == CACHE_BLOB)
    {
      ...
      if (last_record)
      {
        ...
        blob_field->get_image((uchar*)cp, copy->length+sizeof(uchar*),
                              blob_field->charset());
        cp+= copy->length+sizeof(uchar*);
      }
      else
      {
        /* First put down the length of the blob and then copy the data */
        blob_field->get_image((uchar*)cp, copy->length,
                              blob_field->charset());
...
uint JOIN_CACHE::read_record_field(CACHE_FIELD *copy, bool blob_in_rec_buff)
{
  ...
  if (copy->type == CACHE_BLOB)
  {
    ...
    */
    if (blob_in_rec_buff)
    {
      blob_field->set_image((uchar*)pos, copy->length+sizeof(uchar*),
                            blob_field->charset());
      len= copy->length+sizeof(uchar*);
    }
    else
    {
      blob_field->set_ptr((uchar*)pos, (uchar*)pos+copy->length);
...
enum_nested_loop_state JOIN_CACHE_BNL::join_matching_records(bool skip_last)
{
  ...
  */
  ...
  do
  {
    ...
    */
    if (rc == NESTED_LOOP_OK && (!select || !select_skip_record(join->thd, select)))
    {
...
inline bool JOIN_CACHE::check_match(uchar *rec_ptr)
{
  ...
  if (join_tab->select && select_skip_record(join->thd, join_tab->select))
    return FALSE;
  ...
  */
  ...
  do
  {
    ...
    */
    ...
    {
      if (tab->select && select_skip_record(join->thd, tab->select))
        return FALSE;
    }
  }
...
JOIN_CACHE_BKA::init_join_matching_records(RANGE_SEQ_IF *seq_funcs, uint ranges)
{
  ...
  */
  if (!file->inited)
    file->ha_index_init(join_tab->ref.key, join_tab->sorted);
  ...
  while (!(error= file->multi_range_read_next((char **) &key_chain_ptr)))
  {
    if (no_association)
    {
      ...
      */
      ...
      if (!key_search((uchar*)ref->key_buff, ref->key_length, &key_ref_ptr))
        continue;
