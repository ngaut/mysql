...
#endif
char *mysqld_ports_str;
/* Value must match sscanf() in read_mysqld_ports()! */
#define MAX_MYSQLD_PORTS 10
uint mysqld_ports[MAX_MYSQLD_PORTS];
...
/* Client function of close_connections() only.  */
static void shutdown_socket(my_socket &sock)
{
...
{
  static char mysqld_ports_str_buffer[12];
  char	*env;
  if (!mysqld_ports[0] && !opt_disable_networking)
  {					// Get port if not from commandline
    mysqld_ports[0]= MYSQL_PORT;
    ...
    */
    ...
    if ((serv_ptr= getservbyname("mysql", "tcp")))
      mysqld_ports[0]=
          ntohs((u_short) serv_ptr->s_port);    /* purecov: inspected */
#endif
    if ((env = getenv("MYSQL_TCP_PORT")))
      mysqld_ports[0]= (uint) atoi(env);        /* purecov: inspected */
    if (!sprintf(mysqld_ports_str_buffer, "%u", mysqld_ports[0]))
      sql_print_warning("unable to sprintf mysqld_ports");
    mysqld_ports_str= mysqld_ports_str_buffer;
  }
  if (!mysqld_unix_port)
  {
#ifdef __WIN__
...
static void read_mysqld_ports(void)
{
  // Clear just to be sure.
  for (int i= 0; i < MAX_MYSQLD_PORTS; i++)
    mysqld_ports[i]= 0;

  if (!mysqld_ports_str || !mysqld_ports_str[0]) return;

  // Count commas to determine how many ports to expect.
  int commas= 0;
  char *p= mysqld_ports_str;
  ...
  while (*p)
  {
    ...
    {
      sql_print_error("Character error in mysqld_ports '%s'.",
                      mysqld_ports_str);
      unireg_abort(1);
    }
    p++;
  ...
  int expected_ports= commas + 1;
  if (expected_ports > MAX_MYSQLD_PORTS)
  {
    sql_print_error("Number of ports (%d) in mysqld_ports '%s' exceeds "
                    "maximum (%d).",
                    expected_ports, mysqld_ports_str, MAX_MYSQLD_PORTS);
    unireg_abort(1);
  }

  // Needs to match #define MAX_MYSQLD_PORTS.
  int count= sscanf(mysqld_ports_str,
                    "%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
                    &(mysqld_ports[0]),
                    &(mysqld_ports[1]),
                    &(mysqld_ports[2]),
  ...
  if (count != expected_ports)
  {
    sql_print_error("Number of ports (%d) in mysqld_ports '%s' does not "
                    "match expected number (%d).",
                    count, mysqld_ports_str, expected_ports);
    unireg_abort(1);
  }
}
...
static void socket_init(my_socket *sock, ulong bind_addr, const uint port)
{
  ...
  DBUG_ENTER("socket_init");

  LINT_INIT(ret);

  if (port != 0 && !opt_disable_networking && !opt_bootstrap)
  {
    DBUG_PRINT("general", ("IP Socket is %u", port));
    ...
    */
    (void) setsockopt(*sock, SOL_SOCKET, SO_REUSEADDR, (char*)&arg, sizeof(arg));
#endif /* __WIN__ */
    /*
    ...
    */
    ...
    {
      if (((ret= bind(*sock, my_reinterpret_cast(struct sockaddr *) (&IPaddr),
                      sizeof(IPaddr))) >= 0) ||
    ...
    if (listen(*sock, (int) back_log) < 0)
    {
      ...
      sql_print_error("listen() on TCP/IP (Port %u) failed with error %d",
                      port, socket_errno);
      unireg_abort(1);
    }
...
#endif
      (!opt_disable_networking && ip_socks[0] == INVALID_SOCKET))
  {
    select_thread_in_use = 0;
...
#endif
  max_used_connection= (uint) max(max_used_connection, (uint) unix_sock);
#endif
  ++max_used_connection;
  ...
  while (!abort_loop)
  {
    ...
    else
    {
      for (int i= 0;
           ...
           i++)
      {
        if (FD_ISSET(ip_socks[i], &readFDs))
        {
          sock= ip_socks[i];
          flags= ip_flags[i];
          is_ip_sock= true;
      ...
      if (!is_ip_sock)
      {
        /* Not one of our listeners, that is strange. */
        ++bogus_socket_ready;
        continue;
      }
    }
...
enum options_mysqld
{
  ...
  OPT_AUDIT_LOG_REMOTE_CONNECTIONS,
...
struct my_option my_long_options[] =
{
  ...
  {"binlog-row-event-max-size", OPT_BINLOG_ROWS_EVENT_MAX_SIZE,
   ...
   /* max_value */ (longlong) ULONG_MAX, /* sub_size */ 0, /* block_size */ 256,
   /* app_type */ 0
  },
...
each time the SQL thread starts.",
  ...
  {"audit-log-remote-connections", OPT_AUDIT_LOG_REMOTE_CONNECTIONS,
   "Log connections to the server not from localhost or via UNIX socket.",
   &opt_audit_log_remote_connections, 0, 0, GET_BOOL, OPT_ARG, 0, 0, 0, 0, 0, 0},
 ...
 {"log-slow-slave-statements", OPT_LOG_SLOW_SLAVE_STATEMENTS,
  ...
  {"pid-file", OPT_PID_FILE, "Pid file used by safe_mysqld.",
   ...
   REQUIRED_ARG, 0, 0, 0, 0, 0, 0},
  {"port", 'P', "Port number(s) to use for connection or 0 for default to, in "
...
#include "sslopt-longopts.h"
  ...
  {"key_buffer_size", OPT_KEY_BUFFER_SIZE,
   ...
   REQUIRED_ARG, KEY_CACHE_SIZE, 0, (longlong) SIZE_T_MAX, MALLOC_OVERHEAD,
   IO_SIZE, 0},
  {"key_cache_age_threshold", OPT_KEY_CACHE_AGE_THRESHOLD,
   ...
   (GET_ULONG | GET_ASK_ADDR), REQUIRED_ARG,
  ...
  {"max_binlog_cache_size", OPT_MAX_BINLOG_CACHE_SIZE,
   ...
   GET_ULL, REQUIRED_ARG, (longlong) ULONG_MAX, IO_SIZE,
   (longlong) ULONGLONG_MAX, 0, IO_SIZE, 0},
  ...
  {"max_heap_table_size", OPT_MAX_HEP_TABLE_SIZE,
   ...
   REQUIRED_ARG, 16*1024*1024L, 16384, (longlong) MAX_MEM_TABLE_SIZE,
  ...
  {"max_join_size", OPT_MAX_JOIN_SIZE,
   ...
   (longlong) HA_POS_ERROR, 1, (longlong) HA_POS_ERROR, 0, 1, 0},
  ...
  {"myisam_mmap_size", OPT_MYISAM_MMAP_SIZE,
   ...
   GET_ULL, REQUIRED_ARG, (longlong) SIZE_T_MAX, MEMMAP_EXTRA_MARGIN,
   (longlong) SIZE_T_MAX, 0, 1, 0},
  ...
  {"net_retry_count", OPT_NET_RETRY_COUNT,
   ...
   GET_ULONG, REQUIRED_ARG, MYSQLD_NET_RETRY_COUNT,
  ...
  {"query_alloc_block_size", OPT_QUERY_ALLOC_BLOCK_SIZE,
   ...
   REQUIRED_ARG, QUERY_ALLOC_BLOCK_SIZE,
   1024, (longlong) ULONG_MAX, 0, 1024, 0},
#ifdef HAVE_QUERY_CACHE
  ...
  {"tmp_table_size", OPT_TMP_TABLE_SIZE,
   ...
   REQUIRED_ARG, 16*1024*1024L, 1024, (longlong) MAX_MEM_TABLE_SIZE, 0, 1, 0},
  ...
  {"transaction_prealloc_size", OPT_TRANS_PREALLOC_SIZE,
   ...
   REQUIRED_ARG, TRANS_ALLOC_PREALLOC_SIZE,
   1024, (longlong) ULONG_MAX, 0, 1024, 0},
...
SHOW_VAR status_vars[]= {
  ...
  {"Bogus_socket_ready",       (char*) &bogus_socket_ready,     SHOW_LONG},
  ...
  {"Google_connect_failures",  (char*) &google_connect_failures, SHOW_LONG},
  {"Google_connect_not_possible",(char*) &google_connect_not_possible, SHOW_LONG},
  {"Google_connect_tier0",     (char*) &google_connect_tier0,   SHOW_LONG},
  {"Google_connect_tier1",     (char*) &google_connect_tier1,   SHOW_LONG},
  {"Google_connect_tier2",     (char*) &google_connect_tier2,   SHOW_LONG},
  {"Google_connect_tier3",     (char*) &google_connect_tier3,   SHOW_LONG},
  {"Google_connect_tier4",     (char*) &google_connect_tier4,   SHOW_LONG},
  {"Google_connect_tier5",     (char*) &google_connect_tier5,   SHOW_LONG},
  {"Google_connect_tier6",     (char*) &google_connect_tier6,   SHOW_LONG},
  {"Google_connect_tier7",     (char*) &google_connect_tier7,   SHOW_LONG},
  {"Google_connect_tier8",     (char*) &google_connect_tier8,   SHOW_LONG},
  {"Google_connect_tier9",     (char*) &google_connect_tier9,   SHOW_LONG},
  {"Google_fetch_bytes",       (char*) &google_fetch_bytes,     SHOW_LONGLONG},
  {"Google_fetch_bytes_tier0", (char*) &google_fetch_bytes_tier0, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier1", (char*) &google_fetch_bytes_tier1, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier2", (char*) &google_fetch_bytes_tier2, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier3", (char*) &google_fetch_bytes_tier3, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier4", (char*) &google_fetch_bytes_tier4, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier5", (char*) &google_fetch_bytes_tier5, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier6", (char*) &google_fetch_bytes_tier6, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier7", (char*) &google_fetch_bytes_tier7, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier8", (char*) &google_fetch_bytes_tier8, SHOW_LONGLONG},
  {"Google_fetch_bytes_tier9", (char*) &google_fetch_bytes_tier9, SHOW_LONGLONG},
  {"Google_fetch_rows",        (char*) &google_fetch_rows,      SHOW_LONGLONG},
  {"Google_network_io_failures", (char*) &google_network_io_failures,   SHOW_LONG},
  {"Google_fetch",             (char*) &google_fetch,           SHOW_LONGLONG},
  {"Google_fetch_tier0",       (char*) &google_fetch_tier0,   SHOW_LONG},
  {"Google_fetch_tier1",       (char*) &google_fetch_tier1,   SHOW_LONG},
  {"Google_fetch_tier2",       (char*) &google_fetch_tier2,   SHOW_LONG},
  {"Google_fetch_tier3",       (char*) &google_fetch_tier3,   SHOW_LONG},
  {"Google_fetch_tier4",       (char*) &google_fetch_tier4,   SHOW_LONG},
  {"Google_fetch_tier5",       (char*) &google_fetch_tier5,   SHOW_LONG},
  {"Google_fetch_tier6",       (char*) &google_fetch_tier6,   SHOW_LONG},
  {"Google_fetch_tier7",       (char*) &google_fetch_tier7,   SHOW_LONG},
  {"Google_fetch_tier8",       (char*) &google_fetch_tier8,   SHOW_LONG},
  {"Google_fetch_tier9",       (char*) &google_fetch_tier9,   SHOW_LONG},
  {"Google_fetch",             (char*) &google_fetch,           SHOW_LONGLONG},
  {"Google_fetch_microseconds", (char*) &google_fetch_mics, SHOW_LONGLONG},
  {"Google_fetch_microseconds_per", (char*) &google_fetch_mics_per, SHOW_LONGLONG},
  {"Google_nonfetch",          (char*) &google_nonfetch,        SHOW_LONGLONG},
  {"Google_nonfetch_microseconds", (char*) &google_nonfetch_mics, SHOW_LONGLONG},
  {"Google_nonfetch_microseconds_per", (char*) &google_nonfetch_mics_per, SHOW_LONGLONG},
  {"Google_requests",          (char*) &google_requests,        SHOW_LONG},
  {"Google_statsservers_tier0", (char*) &google_statsservers_tier0,   SHOW_LONG},
  {"Google_statsservers_tier1", (char*) &google_statsservers_tier1,   SHOW_LONG},
  {"Google_statsservers_tier2", (char*) &google_statsservers_tier2,   SHOW_LONG},
  {"Google_statsservers_tier3", (char*) &google_statsservers_tier3,   SHOW_LONG},
  {"Google_statsservers_tier4", (char*) &google_statsservers_tier4,   SHOW_LONG},
  {"Google_statsservers_tier5", (char*) &google_statsservers_tier5,   SHOW_LONG},
  {"Google_statsservers_tier6", (char*) &google_statsservers_tier6,   SHOW_LONG},
  {"Google_statsservers_tier7", (char*) &google_statsservers_tier7,   SHOW_LONG},
  {"Google_statsservers_tier8", (char*) &google_statsservers_tier8,   SHOW_LONG},
  {"Google_statsservers_tier9", (char*) &google_statsservers_tier9,   SHOW_LONG},
